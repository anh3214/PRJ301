    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (service.equals("insertTile")) {
                ...
                if (submit == null) {
                    dispath(request, response, "/JSP/InsertTitle.jsp");
                } else {
                    // Get data
            ...
            if (service.equals("searchT")) {
                String id = request.getParameter("id");
                if (id == null) {
                    Vector<tiltes> vector = dao.viewAllTiles();
                    request.setAttribute("listT", vector);
                    dispath(request, response, "/JSP/Client/ClientStore/index.jsp");
                }
                Vector<tiltes> vector = dao.searchPu(id);
                request.setAttribute("listT", vector);
                dispath(request, response, "/JSP/Client/ClientStore/index.jsp");
            }
            if (service.equals("delete")) {
                String id = request.getParameter("titleID");
                if (dao.deleteTile(id) > 0) {
                    Vector<tiltes> vector = dao.viewAllTiles();
