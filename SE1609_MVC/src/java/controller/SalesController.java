    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (service.equals("insertSales")) {
                ...
                if (submit == null) {
                    dispath(request, response, "/JSP/InsertSales.jsp");
                } else {
            ...
            if (service.equals("viewdetail")) {
                try {
                    ...
                    request.setAttribute("storid", StorID);
                    String ord = request.getParameter("ord");
                    String titlid = request.getParameter("titlid");
                    String sql = "select s.ord_num, t.title, s.qty, t.price, s.qty*t.price as Total\n"
                            ...
                            + "where sto.stor_id ='" + StorID + "' and s.ord_num ='" + ord + "' and t.title_id ='" + titlid + "'";
                    ResultSet rs = dao.getData(sql);
                    request.setAttribute("rsDetail", rs);
                    String sql1 = "SELECT status\n"
                            + "FROM sales "
                            + "WHERE stor_id='" + StorID + "' and ord_num='" + ord + "' and title_id='" + titlid + "'";

                    ResultSet rs1 = dao.getData(sql1);
                    request.setAttribute("status", rs1);
                    dispath(request, response, "/JSP/Display/DisplayDetail.jsp");
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
            if (service.equals("searchStaus")) {
                String submit = request.getParameter("submit");
                if (submit == null) {
                    ...
                    String status = request.getParameter("status");
                    int status1 = Integer.parseInt(status);
                    Vector<sales> sal = dao.searchS(status1);
                    request.setAttribute("listSal", sal);
                    dispath(request, response, "/JSP/Client/ClientAdmin/index.jsp");
                }
            }
            ...
            if (service.equals("updateStatus")) {
                ...
                String ord = request.getParameter("ordNum");
                int sta = Integer.parseInt(status);
                DAOstores dao1 = new DAOstores();
                String sql = "update sales \n"
                        + "set status=" + sta + "\n"
                        + "where stor_id='" + id + "' and ord_num ='" + ord + "'";
                ResultSet rs = dao1.getData(sql);

                response.sendRedirect("Home?go=homeA");
            }
            if (service.equals("delete")) {
                String storID = request.getParameter("storID");
                if (dao.deleteSales(storID) > 0) {
                    request.setAttribute("true1", "Delete Done");
                    Vector<sales> vector = dao.viewAll();
                    // pre some other data
                    String titlePage = "Sales manager";
                    String titleTable = "List of sales";
                    ...
                    request.setAttribute("fales", "Delete faile because foreingkey exist!");
                    Vector<sales> vector = dao.viewAll();
                    // pre some other data
                    String titlePage = "Sales manager";
                    String titleTable = "List of sales";
